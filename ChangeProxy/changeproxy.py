...
class ChangeProxy(object):         					# 程序运行：__init__  到  process_request
	...
	def __init__(self):								#初始化
		self.ip_url = "http://ip.jiangxianli.com/api/proxy_ips"			   	#访问代理IP获取地址，此处获得的是json格式，m个代理IP
		self.temp_url = "http://ip.chinaz.com/"    #访问网址，IP查询,用于验证IP是否有效
		self.ip_list = []
	...
	def getIPData(self):                      		#从代理IP处获得是个代理IP，先清空slef.ip_list,再存入self.ip_list的IP和port中。
		...
		for eve_ip in json.loads(temp_data)["data"]["data"]:	    #json.loads(temp_data)["result"]:处理json文件，获取result里面的内容。
			self.ip_list.append({
				"ip":eve_ip["ip"],					#获取ip、port 键的值。
				"port":eve_ip["port"] 
				})

	def changeProxy(self,request):      			#scrapy修改代理IP
